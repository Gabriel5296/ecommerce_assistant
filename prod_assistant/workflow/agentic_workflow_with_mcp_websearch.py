...
class AgenticRAG:
    ...
    def __init__(self, mcp_url: str = "http://localhost:8001/mcp"):
        ...
        # Load MCP tools (works both standalone and under FastAPI)
        self._load_mcp_tools()

    def _load_mcp_tools(self):
        """Load MCP tools, handling both standalone and event-loop contexts."""
        try:
            loop = asyncio.get_running_loop()
            # Already in an event loop (e.g. FastAPI) — schedule as a task
            loop.create_task(self._safe_async_init())
        except RuntimeError:
            # No event loop running (standalone) — use asyncio.run()
